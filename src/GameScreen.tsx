...
export function GameScreen({ onQuit, mode, playerName, playerAvatar = '👨‍🍳' }: GameScreenProps) {
  ...
  const handleProcessItem = (id: string, action: import('./types').ProcessType, amount: number) => {
    ...
    setPrepItems(prev => prev.map(item => {
      ...
      // Нарезка: каждый рез передаёт долю прогресса напрямую (100/число резов),
      // поэтому нужно несколько движений ножом, а не один свайп до готовности.
      if (ing.process === 'cut') {
        const newCut = Math.min(100, item.progress + finalAmount);
        if (newCut >= 100) return { ...item, progress: 100, state: 'ready' };
        return { ...item, progress: newCut, state: 'processing' };
      }
