...
const STEPS = [
  ...
  },
  {
    ...
    body: [
      ...
      { icon: '🎛️', text: 'Крути ручку ВКЛ/ВЫКЛ справа — загорится огонь и конфорка нагреется.' },
      { icon: '🔥', text: 'Чем горячее (полоска нагрева) — тем быстрее жарится. Видишь 💨 пар — хорошо!' },
      { icon: '✅', text: 'Готово — тапни по сковороде, чтобы снять. Не передержи — сгорит! 🟤' },
  ...
  },
  {
    ...
    body: [
      { icon: '🎛️', text: 'Духовка: крути ручку ВКЛ — дверца откидывается, внутри загорается жар.' },
      { icon: '🥧', text: 'Корж, тост, выпечка пекутся сами, пока духовка включена. Тап ускоряет.' },
      { icon: '🥣', text: 'Кастрюля и миксер: вари суп/взбивай крем круговыми движениями пальца 🌀.' },
      { icon: '🥄', text: 'Венчик и пузырьки крутятся в такт помешиванию — мешай ровно по кругу.' },
  ...
  },
  {
    ...
    body: [
      ...
      { icon: '✋', text: 'Перетащи его пальцем на тарелку (или просто тапни — это быстрее).' },
      { icon: '🟢', text: 'Зелёная галочка ✓ и зелёная зона = верный ингредиент, клади сейчас.' },
      { icon: '🔴', text: 'Красный крест ✕ = не из этого блюда. Бросишь — вернётся в «Готово», тарелку не испортит.' },
